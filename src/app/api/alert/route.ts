...
/**
 * POST /api/alert — Triggered by Vercel Cron every Friday at 6am PHT.
 * Fetches the forecast then emails all subscribers.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    const subject = `${arrow} OhmyGas Alert: Fuel prices expected to go ${forecast.direction} ₱${forecast.estimatedChange.toFixed(2)}/L`;

    const crudeInfo = market?.crude
      ? `Crude Oil: $${market.crude.price.toFixed(2)}/barrel`
      : "";
    const forexInfo = market?.forex
      ? `USD/PHP: ₱${market.forex.rate.toFixed(2)}`
      : "";
    ...
    const html = `
      <div style="font-family: system-ui, sans-serif; max-width: 480px; margin: 0 auto;">
        ...
        <div style="background: ${forecast.direction === "up" ? "#fdecea" : forecast.direction === "down" ? "#e8f5ec" : "#fef9e7"}; padding: 20px; border: 1px solid #e5e7eb; border-top: none;">
          ...
          <h2 style="margin: 8px 0 4px; font-size: 18px; color: #2D2D2D;">
            Prices expected to go ${forecast.direction}
          </h2>
          <p style="font-size: 24px; font-weight: bold; margin: 0; color: #2D2D2D;">
            ${forecast.direction === "steady" ? "" : forecast.direction === "up" ? "+" : "-"}₱${forecast.estimatedChange.toFixed(2)}/L
          </p>
          <p style="color: #6b7280; font-size: 14px; margin: 12px 0 0;">
            Effective: ${new Date(forecast.effectiveDate).toLocaleDateString("en-PH", { weekday: "long", month: "short", day: "numeric" })}
          </p>
        </div>
        ...
        <div style="padding: 12px 20px; background: #f9fafb; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
          <p style="font-size: 11px; color: #9ca3af; margin: 0; text-align: center;">
            <a href="https://ohmygas.vercel.app" style="color: #1B7A3D;">View all prices</a> · Confidence: ${forecast.confidence}
          </p>
        </div>
      </div>
    `;
    ...
    return NextResponse.json({
      message: `Alert sent to ${sent}/${subscribers.length} subscribers`,
      sent,
      total: subscribers.length,
      forecast: {
        direction: forecast.direction,
        change: forecast.estimatedChange,
      },
    });
  } catch (error) {
    console.error("Alert error:", error);
    return NextResponse.json(
      { error: "Failed to send alerts", message: error instanceof Error ? error.message : "Unknown" },
      { status: 500 }
    );
  }
}
